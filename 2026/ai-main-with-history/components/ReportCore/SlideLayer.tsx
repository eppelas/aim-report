...
export const SlideLayer: React.FC<SlideLayerProps> = ({ data, onNext, onPrev, nextTitle, isDark }) => {
    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            // SVG Animations - SLOWED DOWN (3-4x slower)
    ...
    return (
        <div ref={containerRef} className={`relative w-full min-h-screen ${bgMain} ${textMain} flex flex-col items-center justify-center overflow-hidden transition-colors duration-500`}>
             ...
             <div className="absolute bottom-10 w-full text-center">
                <p className="font-mono text-[10px] text-neutral-600 uppercase tracking-widest">Next: {nextTitle}</p>
             </div>
        </div>
    );
};
