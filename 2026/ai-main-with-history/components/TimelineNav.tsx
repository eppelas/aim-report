...
export const TimelineNav: React.FC<NavigationProps> = ({ 
  ...
  else if (viewState === 'conclusion') activeNodeIndex = totalNodes - 2; 
  else if (viewState === 'thankyou') activeNodeIndex = totalNodes - 1;   
  ...
  // Logic: Only apply transition after mounting to prevent initial "crawl"
  const transitionClass = isMounted ? 'transition-[opacity,transform] duration-700 ease-out' : '';
  const visibilityClass = (visible && isMounted) 
