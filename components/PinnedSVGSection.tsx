...
interface PinnedSVGSectionProps {
  onOpenReport?: () => void;
}

export const PinnedSVGSection: React.FC<PinnedSVGSectionProps> = React.memo(({ onOpenReport }) => {
  ...
  const layersData = [
    {
      ...
      shifts: "3 SHIFTS",
      desc: "Physics, economics, and power.", 
    ...
    },
    {
      ...
      shifts: "3 SHIFTS",
      desc: "The architecture of meaning and reason.", 
  ...
  // Helper to safely get layer data
  ...
  useEffect(() => {
    ...
    const t = {
        ...
        separateDur: isMobile ? 0.5 : 2.0,
        momentIn: isMobile ? 0.5 : 1.8,
        momentDur: isMobile ? 0.5 : 2.0,
        phase1Out: isMobile ? 1.0 : 3.8,
        divergeStart: isMobile ? 1.2 : 4.0,
        divergeDur: isMobile ? 0.4 : 1.5,
        gapAfterDiverge: isMobile ? 0.4 : 1.5,
    };
    ...
    const ctx = gsap.context(() => {
      ...
      if (signalRedRef.current) gsap.set(signalRedRef.current, { opacity: 0, strokeDasharray: 240, strokeDashoffset: 240 });
      if (signalWhiteRef.current) gsap.set(signalWhiteRef.current, { opacity: 0, strokeDasharray: 240, strokeDashoffset: 240 });
      if (connectionLineRef.current) gsap.set(connectionLineRef.current, { width: 0, opacity: 0 });

      gsap.set(gapGraphRef.current, { opacity: 0, scale: 0.8 });
      const graphPaths = gapGraphRef.current?.querySelectorAll('path, line');
      if (graphPaths && graphPaths.length > 0) gsap.set(graphPaths, { opacity: 0, strokeDasharray: 500, strokeDashoffset: 500 });
      ...
      if (machineChaosRef.current) {
        ...
        if (rings.length > 0) {
            gsap.to(rings, { rotation: 360, duration: 8, repeat: -1, ease: "linear", stagger: { amount: 2, from: "random" } });
        }
      }
      ...
      if (humanBreathRef.current) {
         ...
         if (circles.length > 0) {
            gsap.to(circles, { scale: 1.4, opacity: 0.3, duration: 3, repeat: -1, yoyo: true, ease: "sine.inOut", stagger: 0.5 });
         }
      }
      ...
      tl.fromTo(textDivergeRef.current, 
        { y: -120, scale: 0.8, opacity: 0, visibility: 'hidden' }, 
        {
          visibility: 'visible',
          opacity: 0.5,
          scale: 0.8, 
      ...
      if (gapGraphRef.current) {
         const paths = gapGraphRef.current.querySelectorAll('path, line');
      ...
      tl.to(gapGraphRef.current, { opacity: 0, duration: 0.5 }, gapTime + 4.5);


      // =========================================
      // PHASE 3.5
      // =========================================
      const bridgeTime = gapTime + 5.0;
      ...
      if (machineGroup && humanGroup && gapGroup && quoteGroup) {
        ...
        const signalDuration = 1.0;
        
        if (connectionLineRef.current) {
            tl.to(connectionLineRef.current, { width: "100%", opacity: 0.5, duration: 1 }, manifestoTime + 0.8);
        }

        if (signalRedRef.current && signalWhiteRef.current) {
           tl.to(signalRedRef.current, { opacity: 1, strokeDashoffset: 0, duration: signalDuration, ease: "power1.inOut" }, manifestoTime + 1.2);
           tl.to(coreRightRef.current, { scale: 1.3, duration: 0.2, yoyo: true, repeat: 1 }, manifestoTime + 1.2 + signalDuration);
           tl.to(signalWhiteRef.current, { opacity: 1, strokeDashoffset: 0, duration: signalDuration, ease: "power1.inOut" }, manifestoTime + 1.4 + signalDuration);
           tl.to(coreLeftRef.current, { scale: 1.3, duration: 0.2, yoyo: true, repeat: 1 }, manifestoTime + 1.4 + (signalDuration*2));
           
           tl.to(signalRedRef.current, { opacity: 0, strokeDashoffset: 240, duration: 0 }, manifestoTime + 4.0);
           tl.to(signalRedRef.current, { opacity: 1, strokeDashoffset: 0, duration: 0.6 }, manifestoTime + 4.1);
           tl.to(coreRightRef.current, { scale: 1.2, duration: 0.1, yoyo: true, repeat: 1 }, manifestoTime + 4.7);
        }

        tl.to(coreLeftRef.current, { x: -200, duration: 0.6, ease: "back.out(1.5)" }, manifestoTime + 5.0);
        tl.to(coreRightRef.current, { x: 200, duration: 0.6, ease: "back.out(1.5)" }, manifestoTime + 5.0);
        if (connectionLineRef.current) {
             tl.to(connectionLineRef.current, { width: "120%", opacity: 0, x: -10, duration: 0.5 }, manifestoTime + 5.0);
        }
      ...
      if (signalRedRef.current && signalWhiteRef.current) {
          tl.to([signalRedRef.current, signalWhiteRef.current], { opacity: 0, duration: 0.5 }, manifestoTime + 7.5);
      }
      ...
      // PHASE 4
      // =========================================
      const shiftStartTime = manifestoTime + 8.5; 
  ...
  return (
    ...
    >
      ...
      >
        <svg className="w-full h-full overflow-visible" viewBox="0 0 1000 1000">
          ...
          <g ref={centerShapeRef} transform="translate(500, 500)">
             {/* Updated Signals: Connecting roughly -90 to +90 */}
             <path 
                ref={signalRedRef}
                d="M -90 0 L 90 0" 
                fill="none" 
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
        ...
        <div ref={textIntersectRef} className="absolute w-full text-center z-50 p-4 md:p-0 bg-black/40 backdrop-blur-sm md:bg-transparent md:backdrop-blur-none rounded-xl">
        ...
        <div ref={textDivergeRef} className="absolute w-full text-center z-50 p-4 md:p-0 bg-black/40 backdrop-blur-sm md:bg-transparent md:backdrop-blur-none rounded-xl">
        ...
        {/* UPDATED: Description position lower and removed period */}
        <div ref={textGapDescRef} className="absolute text-center w-full px-4 z-50 top-[35%] md:bottom-[5%] md:top-auto max-w-3xl">
                   ...
                   <div className="h-[300px] relative">
                     {hoveredData && (
                          <div className="absolute top-0 left-0 w-full p-8 border border-white/20 bg-black/80 backdrop-blur-xl rounded-xl animate-fade-in-up transition-all duration-300 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
                             {/* UPDATED: Tightened margin/padding here */}
        ...
        <div ref={textManifestoRef} className="absolute inset-0 flex flex-col items-center justify-center z-30 pointer-events-none">
           ...
           {/* UI Connection Line */}
           <div ref={connectionLineRef} className="absolute w-0 h-[1px] bg-gradient-to-r from-[#DC2626] via-white to-white top-[calc(50%-100px)] md:top-[calc(50%-80px)] opacity-0 pointer-events-none z-0"></div>

           <div className="relative flex w-full max-w-4xl justify-center items-center mb-12 px-2 md:px-4 pointer-events-none flex-row gap-4 md:gap-8 z-10">
              
              <div className="machine-group flex flex-col items-end z-10 flex-1 max-w-[45%] md:max-w-[280px]">
              ...
              <div className="human-group flex flex-col items-start text-left z-10 flex-1 max-w-[45%] md:max-w-[280px]">
        ...
        <div ref={textReportRef} className="absolute inset-0 z-50 flex flex-col items-center justify-center pointer-events-auto">
          ...
          <div className="flex flex-col items-center justify-center flex-1 pt-20 md:pt-0">
              <h2 ref={reportTitleRef} className="text-xs font-mono font-bold text-neutral-500 uppercase tracking-widest mb-6 hidden md:block">This is the report.</h2>
          ...
          <div className="absolute bottom-12 flex flex-col items-center w-full relative z-10 pb-40">
            <button ref={btnReportRef} 
              ...
              onClick={onOpenReport} // UPDATED CALLBACK
            >
