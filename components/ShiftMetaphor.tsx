...
interface ShiftMetaphorProps {
  ...
  isDark: boolean;
  centered?: boolean;
  hoverTrigger?: boolean; // Backward compatibility
}

// Shared Atmosphere Component
...
export const ShiftMetaphor: React.FC<ShiftMetaphorProps> = ({ id, isDark, centered, hoverTrigger }) => {
  ...
  const renderContent = () => {
    switch (id) {
      case "01": // Energy Wall - Specific Geometry
        return (
          <g>
  ...
  // If centered (or hoverTrigger legacy prop) is true, remove translation
  const isCentered = centered || hoverTrigger;
  const transformAttr = isCentered ? undefined : "translate(500, 700)";

  return (
    <g ref={containerRef} transform={transformAttr}>
       {renderContent()}
    </g>
  );
};